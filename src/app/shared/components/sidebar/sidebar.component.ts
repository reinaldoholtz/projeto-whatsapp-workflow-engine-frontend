      ...
      <div class="flex items-center gap-3 px-5 py-5 border-b border-slate-700/50">
        ...
        <div class="flex-1 min-w-0">
          ...
          @if (auth.isMaster()) {
            ...
            <p class="text-slate-400 text-xs">{{ auth.user()?.databaseName ?? 'Tenant Padrão' }}</p>
          }
        </div>
      </div>
      ...
      <div class="border-t border-slate-700/50 p-3">
        <div class="flex items-center gap-3 px-2 py-2">
          <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
            [ngClass]="auth.isMaster() ? 'bg-purple-600' : 'bg-primary-600'">
...
export class SidebarComponent {
  ...
  navItems: NavItem[] = [
    ...
    { label: 'Disparar Leads',     icon: 'send',            route: '/lead-disparo',
      dividerBefore: true                                                                       },
    { label: 'Histórico Disparos', icon: 'history',         route: '/lead-disparo/historico',
      subLabel: 'Arquivos e resultados'                                                        },

    // ── Admin + Master ──────────────────────────────────────────────────────
    { label: 'Workflows',          icon: 'account_tree',    route: '/workflows',
      adminOnly: true, dividerBefore: true                                                     },
    { label: 'Canais WhatsApp',    icon: 'perm_phone_msg',  route: '/meta-phones',
      adminOnly: true                                                                           },
    { label: 'Usuários',           icon: 'manage_accounts', route: '/users',
      adminOnly: true                                                                           },

    // ── Master only ─────────────────────────────────────────────────────────
    { label: 'Tenants',            icon: 'domain',          route: '/tenants',
      masterOnly: true, dividerBefore: true, subLabel: 'Gestão global'                        },
  ...
  isVisible(item: NavItem): boolean {
    ...
    if (item.adminOnly)   return this.auth.isAdmin();   // ADMIN ou MASTER
    return true;
  }
